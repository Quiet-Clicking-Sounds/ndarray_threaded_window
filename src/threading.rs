...
use ndarray::{s, Array3, AssignElem, Axis, Dim, Ix, Ix3, Shape};
use num_traits::identities::Zero;

use crate::window_functions;
use crate::window_functions::fast_unsigned_integer::NumConv;
...
impl WindowShape {
    ...
    pub fn array_size<U>(self, ar: &Array3<U>) -> (Shape<Dim<[Ix; 3]>>, Dim<[Ix; 3]>) {
        ...
        assert!(
            ar.shape()[0] >= w.0,
            "Error in WindowShape: First Dimension of Array must be larger than of the Window"
        );
        assert!(
            ar.shape()[1] >= w.1,
            "Error in WindowShape: Second Dimension of Array must be larger than of the Window"
        );
        assert!(
            ar.shape()[2] >= w.2,
            "Error in WindowShape: Third Dimension of Array must be larger than of the Window"
        );
        let sh = ar.raw_dim();
...
///
/// ```
fn apply_over_window<T>(arr: Array3<T>, win_size: WindowShape, func: WinFunc<T, Ix3>) -> Array3<T>
where
    T: Zero,
    T: NumConv,
    T: Clone,
...
mod tests {
    ...
    use crate::threading::{thread_apply_over_window, WindowShape};
    use window::window_methods::*;

    use crate::window_functions;
    use crate::window_functions::fast_unsigned_integer::{fast_std, stdev_ddof_0, stdev_ddof_1};
    ...
    fn u8_test_with_std_array_rms_u64() {
        ...
        let out = thread_apply_over_window(test_array, win_shape, fast_std);
